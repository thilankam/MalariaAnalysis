...
GDPData %>% select(GDP_In_Billions_of_USD,Year,Country_or_Area) %>% filter(GDPData$Country_or_Area=='Yemen') %>% head(10)
...
ggplot(data = Yemen, aes(x=Yemen$Year.s.,y=Yemen$Value))+geom_point(color='red',size=3)+xlab("Year")+ylab("Value")+ggtitle("Number of Confirmed Malaria Cases in Yemen")+theme_bw(base_size = 18)
ggplot(data = Yemen, aes(x=Year.s.,y=Value))+geom_point(color='red',size=3)+xlab("Year")+ylab("Value")+ggtitle("Number of Confirmed Malaria Cases in Yemen") #+theme_bw(base_size = 18)
ggplot(data = Yemen, aes(x=Year.s.,y=Value))+geom_line(color='red')+geom_point(color='blue',size=3)+xlab("Year")+ylab("Value")+ggtitle("Number of Confirmed Malaria Cases in Yemen") #+theme_bw(base_size = 18)
ggplot(data = Yemen, aes(x=Year.s.,y=Value))+geom_line(color='red')+geom_point(color='blue',size=3)+xlab("Year")+ylab("Value")+ggtitle("Number of Confirmed Malaria Cases in Yemen")+xlim(2000,2014)+ylim(8000,140000)
ggplot(data = Yemen, aes(x=Year.s.,y=Value))+geom_line(color='red')+geom_point(color='blue',size=2)+xlab("Year")+ylab("Value")+ggtitle("Number of Confirmed Malaria Cases in Yemen")+xlim(2000,2014)+ylim(8000,140000)+theme_gray(base_size = 20)
ggplot(data = Yemen, aes(x=Year.s.,y=Value))+geom_line(color='red')+geom_point(color='blue',size=2)+xlab("Year")+ylab("Value")+ggtitle("Number of Confirmed Malaria Cases in Yemen")+xlim(2000,2014)+ylim(8000,140000)+theme(axis.title.x = element_text(angle = 45))

is.na(Yemen)
ggplot(data = MalariaData,aes(x=Year.s.,y=Value,group=Country.or.Area))+geom_line()

# Afganistan
Afghanistan <- MalariaData %>% select(Country.or.Area,Value,Year.s.) %>% filter(Country.or.Area=='Afghanistan')
dim(Afghanistan)
